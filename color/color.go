// Original from github.com/mattn/go-colorable
package color
...
var (
	...
	colorsCache   = make(map[int]*Color)
	colorsCacheMu sync.Mutex // protects colorsCache
)

func getCachedColor(p int) *Color {
	var colorsCache = make(map[int]*Color)
	var colorsCacheMu sync.Mutex
	colorsCacheMu.Lock()
...
func colorString(format string, p int, a ...interface{}) string {
	c := getCachedColor(p)
...
func colorPrint(format string, p int, a ...interface{}) {
	c := getCachedColor(p)
...
func (c *Color) attrExists(a int) bool {
	for _, attr := range c.params {
		if attr == a {
			return true
		}
